...
<head>
    ...
    <style>
        ...
        .candidate-action-form input,
        .candidate-action-form select,
        .candidate-action-form textarea { border-radius:.75rem; outline:none; transition:border-color .15s,box-shadow .15s; }
        .candidate-action-form input:focus,
        .candidate-action-form select:focus,
        .candidate-action-form textarea:focus { border-color:#2563eb; box-shadow:0 0 0 4px rgba(37,99,235,.1); }
    </style>
</head>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-4 lg:p-6">
                <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <a href="<?= base_url('Recruitment/evaluation') ?>" class="mb-4 inline-flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-blue-700">
                            <i data-lucide="arrow-left" class="h-4 w-4"></i>
                            Evaluation Queue
                        </a>
                        <p class="mb-2 text-xs font-extrabold uppercase tracking-[.16em] text-blue-600">Candidate Profile</p>
                        <div class="flex items-center gap-3"><span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-lg font-black text-blue-600"><?= esc(substr($candidateName, 0, 1)) ?></span><div><h1 class="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl"><?= esc($candidateName) ?></h1><p class="mt-1 text-sm text-slate-500"><?= esc($candidateEmail) ?></p></div></div>
                        <div class="mt-3 flex flex-wrap items-center gap-2 text-sm text-slate-500">
                            <span><?= esc($application['job_title'] ?? '-') ?></span>
                            <span class="h-1.5 w-1.5 rounded-full bg-slate-300"></span>
                            <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700"><?= esc($status) ?></span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="<?= $hasResume ? base_url('Recruitment/applications/resume/' . $application['application_id']) : '#' ?>"
                            target="_blank"
                            class="<?= $hasResume ? 'bg-white text-slate-700 hover:bg-slate-50' : 'pointer-events-none bg-slate-100 text-slate-300' ?> inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-bold">
                            <i data-lucide="file-text" class="h-4 w-4"></i>
                            View Resume
                        </a>
                        <a href="<?= $hasResume ? base_url('Recruitment/applications/resume-download/' . $application['application_id']) : '#' ?>"
                            class="<?= $hasResume ? 'bg-blue-600 text-white hover:bg-blue-700' : 'pointer-events-none bg-slate-200 text-slate-400' ?> inline-flex h-10 items-center gap-2 rounded-xl px-4 text-sm font-bold">
                ...
                <div class="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_380px]">
                    <section class="space-y-5">
                        ...
                        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                                ...
                                <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700"><?= count($roundEvaluations ?? []) ?> completed</span>
                            </div>
                                    ...
                                    <?php foreach ($roundEvaluations as $roundEvaluation): ?>
                                        <article class="rounded-xl border border-slate-200 p-4">
                                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                ...
                                                <span class="text-2xl font-black text-blue-700"><?= (int) $roundEvaluation['total_score'] ?><span class="text-sm text-slate-400">/100</span></span>
                                            </div>
                                            <div class="mt-4 grid grid-cols-3 gap-2 text-center">
                                                <?php foreach ([['Technical', 'technical_score'], ['Communication', 'communication_score'], ['Culture', 'culture_score']] as [$label, $field]): ?>
                                                    <div class="rounded-xl bg-slate-50 p-3"><p class="text-xs text-slate-500"><?= $label ?></p><p class="mt-1 font-bold text-slate-900"><?= (int) $roundEvaluation[$field] ?></p></div>
                                                <?php endforeach; ?>
                                            </div>
                    ...
                    <aside class="space-y-5">
                        <form method="post" action="<?= base_url('Recruitment/applications/shortlist/' . $application['application_id']) ?>" class="candidate-action-form overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <?= csrf_field() ?>
                            <div class="border-b border-slate-200 px-5 py-4">
                            ...
                            <div class="p-5">
                                ...
                                <textarea name="screening_notes" rows="3" class="mt-2 w-full border border-slate-300 px-3 py-2 text-sm"><?= esc($application['screening_notes'] ?? '') ?></textarea>
                                <button type="submit" class="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white hover:bg-emerald-700">
                        ...
                        <form method="post" action="<?= base_url('Recruitment/applications/schedule/' . $application['application_id']) ?>" class="candidate-action-form overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <?= csrf_field() ?>
                            <div class="border-b border-slate-200 px-5 py-4">
                            ...
                            <div class="space-y-4 p-5">
                                ...
                                <button type="submit" class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-pink-600 px-4 text-sm font-bold text-white hover:bg-pink-700">
                        ...
                        <form method="post" action="<?= base_url('Recruitment/applications/evaluate/' . $application['application_id']) ?>" class="candidate-action-form overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <?= csrf_field() ?>
                            <div class="border-b border-slate-200 px-5 py-4">
                            ...
                            <div class="space-y-4 p-5">
                                ...
                                <button type="submit" class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 px-4 text-sm font-extrabold text-white shadow-lg shadow-blue-100 hover:from-blue-700 hover:to-blue-800">
